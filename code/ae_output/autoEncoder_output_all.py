...
def process_angle_data(file_path):
    ...
    return adjusted_angles.reshape(-1, 1).T  # 轉置以符合原程式碼的輸入格式

def process_single_file(input_file_path, model, base_output_dir):
    """處理單個檔案的函數"""
    ...
    output_file_path = os.path.join(output_dir, 'processed_output.xlsx')
    output_fig_path = os.path.join(output_dir, 'processed_output.png')
    ...
    # 使用Savitzky-Golay濾波器
    filter_data = savgol_filter(full_original_data, 101, 3, mode='nearest')
    ...
    # # output_df.columns = [output_file_name]
    # # output_df.to_excel(output_file_path, index=False)
    # processed_part = denormalized_predictions[0]
    # original_remaining_part = full_original_data[0, end_idx:]

    # combined_data = np.concatenate([processed_part, original_remaining_part])
    ...
    # 準備完整的數據：前面的原始數據 + 處理的部分 + 後面的原始數據
    processed_data = np.concatenate([
        full_original_data[0, :start_idx], 
        processed_part, 
        full_original_data[0, end_idx:]
    ])

    output_df = pd.DataFrame({
        'Processed_Output': processed_data,
        'Original_Full_Signal': full_original_data[0]
    })
    output_df.to_excel(output_file_path, index=False)
    print(f"結果已儲存至 {output_file_path}")
    ...
    # 繪製完整原始數據
    plt.plot(x_full, full_original_data[0], label='Original Data', alpha=0.5, color='gray')

    # 繪製濾波後的數據
    plt.plot(x_full, filter_data[0], label='Filtered Data', alpha=0.7, color='green')

    # 突出顯示處理的部分
    plt.plot(x_full[start_idx:end_idx],
            full_original_data[0, start_idx:end_idx],
            label='Selected Range',
            color='blue',
            alpha=0.7)

    # 繪製預測結果
    plt.plot(x_pred, denormalized_predictions[0],
...
def main():
    ...
    for file_path in angle_files:
        try:
            process_single_file(file_path, model, base_output_dir=None)  # 移除 base_output_dir 參數
        except Exception as e:
