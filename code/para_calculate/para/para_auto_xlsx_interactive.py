...
def analyze_folder(folder_path):
    ...
    """
    分析特定資料夾中的數據並返回結果，支持交互式選擇起始索引
    """
    # 讀取數據
    file_hpe = r'processed_output.xlsx'
    ...
    plt.title('Click to Select Start Index')
    plt.xlabel('Time (s)')
    ...
    def on_click(event):
        if event.button == 1:  # 左鍵點擊
            start_idx[0] = np.argmin(np.abs(time_zeroed - event.xdata))
            plt.close()
    ...
    # 檢查是否選擇了起始索引
    if start_idx[0] is None:
        start_idx[0] = 0  # 預設為0
    
    print(f"Selected start index: {start_idx[0]}")

    # 從選定的起始點開始處理數據
    time_zeroed = time_zeroed[start_idx[0]:]
    hpe = hpe[start_idx[0]:]
...
def process_folders(base_folder):
    ...
    for folder_name in os.listdir(base_folder):
        ...
        for fname in os.listdir(folder_path):
            ...
            if os.path.isdir(fname_path) and os.path.exists(os.path.join(fname_path, 'processed_output.xlsx')):
                try:
