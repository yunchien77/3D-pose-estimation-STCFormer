...
def export_to_excel(folder_name, data_dict, excel_path='exponential_results_elec.xlsx'):
    """
    將分析結果輸出到Excel檔案
    """
    row_order = [
...
#         fig = plt.figure(figsize=(20, 12))
#         plt.plot(data, 'b-', label='Original Data', alpha=0.7)
#         plt.plot(all_peaks, all_values, 'go', label='All Selected Peaks')
#         for i, (peak, value) in enumerate(zip(all_peaks, all_values)):
#             plt.annotate(f'Peak {i+1}\ny={value:.2f}', 
#                         xy=(peak, value),
#                         xytext=(10, 10),
#                         textcoords='offset points')
#         plt.title('Final Selected Peaks')
#         plt.legend()
#         plt.grid(True)
#         plt.show()
#         plt.close(fig)
...
def main():
    base_folder = r"D:/para/021"  # 請修改為你的資料夾路徑
