...
def parse_title(title):
    ...
    try:
        ...
        angle = 'a' if 'a' in title else 'b'  # 45度或90度
        
        parts = title.split('_')
        pose_num = parts[2]  # 取得_1或_2
        
        # 建立基本識別碼（不含重複測量次數）
        base_id = f"{subject_id}_{side}_{leg_type}{angle}_{pose_num}"
...
def main():
    input_file = Path("../../exponential_results_3d.xlsx")
