...
def load_and_process_data(parameter_file, fmas_file):
    ...
    for column in params_df.columns:
        ...
        if '_s' in column.lower():
            ...
            try:
                # 獲取對應的 p1 值
                p1_value = float(params_df.loc['p1', column])
                ...
                if len(fmas_score) > 0:
                    ...
                    print(f"Successfully processed: Subject {subject_num}, p1={p1_value}, FMAS={fmas_score}")
                
            except Exception as e:
                print(f"Error processing subject {subject_num}: {str(e)}")
                continue
    
    if not p1_values or not fmas_values:
        raise ValueError("No valid data pairs were found. Please check your input files.")
    
    return p1_values, fmas_values

def perform_correlation_analysis(p1_values, fmas_values):
    """
    進行 Kendall 相關性分析
    ...
    Returns:
        相關係數和 p 值
    """
    # 使用 Kendall tau 替代 Spearman
    correlation, p_value = stats.kendalltau(p1_values, fmas_values)
...
def analyze_data(parameter_file, fmas_file):
    ...
    """
    # 載入和處理數據
    p1_values, fmas_values = load_and_process_data(parameter_file, fmas_file)
    
    # 進行相關性分析
    correlation, p_value = perform_correlation_analysis(p1_values, fmas_values)
...
if __name__ == "__main__":
    try:
        parameter_file = r"C:\Users\cherr\anaconda3\envs\STCformer\STCFormer-main\analysis_results.xlsx"
