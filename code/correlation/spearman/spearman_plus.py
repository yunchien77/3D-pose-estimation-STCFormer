...
def group_and_process_measurements(params_df):
    ...
    # 結構: measurements[subject_id][angle][pose] = [values]
    ...
    for column in params_df.columns:
        ...
        if parsed and parsed['leg_type'] == 's':
            try:
                value = float(params_df.loc['Decay rate (b)', column])
...
def load_and_process_data(parameter_file, fmas_file):
    ...
    for subject_id, angles in processed_params.items():
        fmas_score = fmas_df.loc[fmas_df['Subject'] == subject_id, 'emas'].values
...
if __name__ == "__main__":
    try:
        ...
        # parameter_file = r"C:\Users\cherr\anaconda3\envs\STCformer\STCFormer-main\analysis_results_hpe_v3.xlsx"
        fmas_file = r"C:\Users\cherr\anaconda3\envs\STCformer\STCFormer-main\code\mas_table.xlsx"
        output_file = "spearman_analysis_results.xlsx"
        
        correlation_results, detailed_results = analyze_data(
